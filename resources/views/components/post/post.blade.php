<div class="card ">
    <div class="card-header p-4">
        ...
        <a href="#" class="font-bold ltr:ml-1 rtl:mr-1">{{ $post->owner->userName }}</a>
    </div>
    <div class="card-body ">
        <div class="max-h-[35rem]  overflow-hidden">
            <img class="w-full object-fit-cover" src={{ asset('storage/' . $post->image) }} alt="{{ $post->description }}">
        </div>
        <div class="flex flex-col px-4 gap-4 my-4">
            ...
            @if ($post->comments->count() > 0)
                <a class="font-blod text-sm text-gray-500" href="{{ route('show_post', $post->slug) }}">View all {{ $post->comments->count() }} Comments</a>
            @endif
